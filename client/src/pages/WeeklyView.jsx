  ...
  return (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', padding: '16px 20px', marginBottom: '16px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
        ...
        {!readOnly && !editing && <button onClick={() => { setDraft(content); setEditing(true); }} style={{ padding: '4px 12px', borderRadius: '6px', fontSize: '12px', background: 'var(--surface2)', border: '1px solid var(--border2)', color: 'var(--text2)', cursor: 'pointer' }}>편집</button>}
        {editing && (
      ...
      {editing ? (
        <textarea autoFocus value={draft} onChange={e => setDraft(e.target.value)} placeholder="이번 주 계획, 목표, 메모를 자유롭게 작성하세요..."
        ...
        <div onClick={() => { if (!readOnly) { setDraft(content); setEditing(true); } }} style={{ fontSize: '13px', color: 'var(--text2)', lineHeight: '1.7', whiteSpace: 'pre-wrap', wordBreak: 'break-word', cursor: readOnly ? 'default' : 'pointer', minHeight: '40px' }}>{content}</div>
      ) : (
        <div onClick={() => { if (!readOnly) { setDraft(''); setEditing(true); } }} style={{ fontSize: '13px', color: 'var(--text3)', cursor: readOnly ? 'default' : 'pointer', minHeight: '40px', fontStyle: 'italic' }}>
