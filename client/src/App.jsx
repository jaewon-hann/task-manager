    ...
    <div style={{ position: 'fixed', top: '20px', right: '20px', zIndex: 9999, display: 'flex', flexDirection: 'column', gap: '8px' }}>
      {toasts.map(t => (
        <div key={t.id} style={{
          ...
          background: t.type === 'error' ? 'rgba(240,96,96,0.95)' : t.type === 'warning' ? 'rgba(247,200,67,0.95)' : 'rgba(78,204,163,0.95)',
          color: t.type === 'warning' ? '#1a1d28' : '#fff',
          maxWidth: '320px', animation: 'slideIn 0.2s ease',
        }}>
          {t.message}
        </div>
      ))}
...
export default function App() {
  ...
  useEffect(() => {
    if (user) api.users.list().then(data => setUsers(data.filter(u => u.is_active))).catch(() => {});
  }, [user]);
  ...
  const navigate = (pageId, params = {}) => { setPage(pageId); setPageParams(params); };

  if (!user) return <><Login onLogin={handleLogin} /><Toast /></>;
      ...
      <div style={s.layout}>
        <aside style={s.sidebar}>
          <div style={s.logoArea}>
            ...
            <div ref={dropdownRef} style={{ position: 'relative', marginTop: '12px' }}>
              <button onClick={() => setDropdownOpen(v => !v)} style={{
                ...
                <span style={{ display: 'flex', alignItems: 'center', gap: '7px', overflow: 'hidden' }}>
                  ...
                  }}>{displayUser.name[0]}</span>
                  <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {isViewingOther ? `${displayUser.name}의 워크플레이스` : '내 워크플레이스'}
                  </span>
                </span>
                <span style={{ fontSize: '10px', flexShrink: 0, marginLeft: '4px' }}>{dropdownOpen ? '▲' : '▼'}</span>
              </button>
              ...
              {dropdownOpen && (
                <div style={{
                  ...
                  <div onClick={() => handleSelectWorkspace(null)} style={{
                  ...
                  >
                    <div style={{ width:'26px',height:'26px',borderRadius:'50%',background:'rgba(111,106,248,0.2)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'11px',fontWeight:'700',color:'var(--accent)',flexShrink:0 }}>{user.name[0]}</div>
                    <div>
                      <div style={{ fontSize:'12px',fontWeight:'500',color:!isViewingOther?'var(--accent)':'var(--text)' }}>{user.name} (나)</div>
                  ...
                  <div style={{ maxHeight: '200px', overflowY: 'auto' }}>
                    {users.filter(u => u.id !== user.id).map(u => (
                      <div key={u.id} onClick={() => handleSelectWorkspace(u)} style={{
                        padding:'10px 12px',cursor:'pointer',display:'flex',alignItems:'center',gap:'8px',
                      ...
                      >
                        <div style={{ width:'26px',height:'26px',borderRadius:'50%',background:'rgba(78,204,163,0.15)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'11px',fontWeight:'700',color:'var(--teal)',flexShrink:0 }}>{u.name[0]}</div>
                        <div>
                          <div style={{ fontSize:'12px',fontWeight:'500',color:targetUser?.id===u.id?'var(--amber)':'var(--text)' }}>{u.name}</div>
            ...
            {isViewingOther && (
              <div style={{ marginTop:'8px',padding:'6px 10px',borderRadius:'6px',background:'rgba(247,200,67,0.08)',border:'1px solid rgba(247,200,67,0.2)',fontSize:'11px',color:'var(--amber)',textAlign:'center' }}>
                👁 조회 전용
              </div>
            )}
          </div>
          ...
          <nav style={s.nav}>
            ...
            {!isViewingOther && (
              <>
                <button style={s.navItem(page==='settings')} onClick={() => navigate('settings')}
                ...
                >
                  <span style={s.navIcon}>⚙</span>설정
                </button>
                {user.role==='admin' && (
                  <button style={s.navItem(page==='team')} onClick={() => navigate('team')}
                  ...
                  >
                    <span style={s.navIcon}>👥</span>팀원 관리
                  </button>
                )}
              </>
            )}
          ...
          <div style={s.footer}>
            <div style={{ display:'flex',alignItems:'center',gap:'8px',marginBottom:'8px' }}>
              <div style={{ width:'28px',height:'28px',borderRadius:'50%',background:'rgba(111,106,248,0.2)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'12px',fontWeight:'700',color:'var(--accent)',flexShrink:0 }}>{user.name[0]}</div>
            ...
            <button onClick={handleLogout} style={{ width:'100%',padding:'6px',borderRadius:'6px',fontSize:'11px',background:'transparent',border:'1px solid var(--border)',color:'var(--text3)',cursor:'pointer' }}
